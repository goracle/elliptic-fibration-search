...
def _project_base_points_from_globals(current_x=None, current_y=None, p: Optional[int] = None):
    ...
    data_pts = _resolve_project_symbol('DATA_PTS_GENUS2', default=[] )
    yfun = _resolve_project_symbol('get_y_unshifted_genus2', default=None)
    finite_field = _resolve_project_symbol('FINITE_FIELD', default=None)
...
def _run_markov_mumford_search_for_point(
    *,
    ...
    """Call the legacy Mumford search in its native finite-field mode.

    The Markov path should not go through the standard lattice pipeline.
    Instead, it should consume the Mumford residues/candidate x-values that
    this routine produces.
    """
    # Keep the call site simple: let the legacy routine build its own modular
    # data and residue structure, then normalize the return value for the walker.
    result = run_mumford_search(
    ...
    if False:
        ...
        print(rhs_list)
        print(shift)
        print(rationality_test_func)
        print(coeffs_genus2)
        print(x_b)
        print(shifted_coeffs)

    return _collect_mumford_candidate_x_values(result)
...
def make_project_markov_search_fn(
    *,
    ...
    """Build a search function that rebuilds the tower for every xi/current point.

    In Markov mode we now route through the legacy Mumford search directly,
    because that path is already the finite-field-native one and returns the
    residue objects needed to identify x_j candidates.
    """
    coeffs_genus2 = coeffs_genus2 if coeffs_genus2 is not None else _resolve_project_symbol('COEFFS_GENUS2', required=True)
    if p is None:
        p = _resolve_project_symbol('FINITE_FIELD', default=None)
    if prime_pool is None:
        prime_pool = _resolve_project_symbol('PRIME_POOL', required=True)
    if num_workers is None:
        num_workers = int(_resolve_project_symbol('NUM_WORKERS', default=1))
    if all_found_x is None:
        all_found_x = set()

    def search_fn(xi=None, current_x=None, n=None, seed=None, current_point=None, walker=None, **kwargs):
        # Resolve the current point. Prefer the explicit current_point from the walker.
        ...
        # One-dimensional search vectors, as requested: [n], [n+1], ..., [max_n]
        n0 = int(n or 1)
        ...
        # Route through Mumford, not the standard lattice path.
        result = _run_markov_mumford_search_for_point(
            cd=ctx['cd'],
        ...
        print("resulting xs", result)
        # result is already a list of xj values
        xs = [x for x in result if x is not None]
        result = {
            'candidates': [{'xj': x} for x in xs],
            'candidate_xs': set(xs),
        }

        # Normalization is already done in the helper, but keep the result shape explicit.
        ...
        result.setdefault('candidate_xs', set())
        result.setdefault('new_sections', [])
        result.setdefault('precomputed_residues', precomputed_residues)
        result.setdefault('stats', None)

        result['tower_context'] = {
            'xi': x_here,
            'yi': y_here,
            'shift': ctx['shift'],
            'prime_pool_size': len(prime_pool),
            'section_count': len(ctx['current_sections']),
            'vec_count': len(vecs),
            'mumford_mode': True,
        }
...
def _normalize_markov_mumford_result(result: Any, fallback_step: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    """
    Normalize the legacy Mumford-search return payload into walker-friendly form.
    ...
    def _candidate_from_x(x, source='mumford_residue'):
        return {'xj': x, 'source': source}

    if result is None:
        return out

    # Already structured dict: preserve it, but enforce the contract.
    if isinstance(result, dict):
        ...
        if 'found_xs' in result:
            out['found_xs'] = _as_set_maybe(result.get('found_xs'))
        if 'candidate_xs' in result:
            out['candidate_xs'] = _as_set_maybe(result.get('candidate_xs'))

        if 'candidates' in result and result['candidates'] is not None:
            if isinstance(result['candidates'], (list, tuple)):
                out['candidates'] = list(result['candidates'])
            else:
                out['candidates'] = [result['candidates']]

        # Pull x-values out of structured candidates.
        ...
        if not out['candidate_xs']:
            ...
            if xs:
                ...
                out['candidates'] = [_candidate_from_x(x) for x in xs]
        ...
        if not out['candidate_xs'] and fallback_step is not None:
            ...
            if xs:
                out['candidate_xs'] = set(xs)
                out['candidates'] = [_candidate_from_x(x, source='fallback_step') for x in xs]

        if out['found_xs']:
            out['candidate_xs'].update(out['found_xs'])

        if not out['candidates'] and out['candidate_xs']:
            out['candidates'] = [_candidate_from_x(x) for x in out['candidate_xs']]
    ...
    # Tuples/lists: scan every slot, do not assume a fixed layout.
