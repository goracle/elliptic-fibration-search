...
all polynomial arithmetic (section ladder, point accumulation, diff poly,
root-finding) to Julia/Oscar.

When section_poly_dict is present (FINITE_FIELD / non-constant-denom path):
  - _build_vecs_and_rhs: builds only (v_tuple, v_coeffs) pairs and RHS int
    lists.  Zero Sage point arithmetic — no LargePrimeMockPoint, no GF(p)(m).
  - Julia does: ladder → Pm accumulation → diff_poly → roots.

When section_poly_dict is absent (constant-denom / old path):
  - _build_tasks_and_rhs: original loop (Sage point accumulation, diff poly
...
def _call_julia_server(prime_list, tasks_by_prime, rhs_by_prime,
    ...
    """
    Serialize and send one task to the persistent Julia server, return raw JSON.

    When vecs_by_prime + section_poly_dict are provided (new path), Julia does
    ladder + accumulation + diff + roots.  tasks_by_prime is ignored.

    When only tasks_by_prime is provided (old path), Julia does root-finding
    on pre-computed diff_coeffs.
    """
    rhs_serial = {
    ...
    if vecs_by_prime is not None and section_poly_dict is not None:
        ...
        vecs_serial = {
            str(p): [[list(v_tuple), list(v_coeffs)] for v_tuple, v_coeffs in items]
            for p, items in vecs_by_prime.items()
        }

        payload = json.dumps({
            "prime_list":    list(prime_list),
            "section_polys": section_serial,
            "vecs":          vecs_serial,
            "rhs":           rhs_serial,
        }, cls=_SageEncoder)
    else:
        # Old path: send pre-computed tasks.
        tasks_serial = {
            str(p): [
...
def _build_vecs_and_rhs(
    prime_list, vecs_lll, rhs_modp_list, vecs_list, const_val_int, debug,
):
    """
    Build:
      vecs_by_prime[p]  : list of (v_tuple, v_coeffs)   — just int lists, no Sage
      rhs_by_prime[p]   : list of (num_coeffs, den_coeffs)

    Julia will do ladder + Pm accumulation + diff poly + roots.
    """
    vecs_by_prime = {}
    rhs_by_prime  = {}
    ...
    for p in prime_list:
        ...
        # RHS reconstruction (same as old path)
        rhs_reconstruction = []
        ...
        # Vecs: just (v_tuple, v_coeffs) — no point arithmetic
        ...
        for v_idx, v_tuple in enumerate(vecs_list):
            ...
            if v_idx >= len(p_vecs):
                continue
            v_coeffs = [int(c) for c in p_vecs[v_idx]]
            if all(c == 0 for c in v_coeffs):
                continue
            items.append((v_tuple, v_coeffs))

        assert items, f"No vecs generated for p={p} — configuration error"
...
def _build_tasks_and_rhs(
    ...
    """
    Original task-generation loop: accumulates Pm in Sage GF(p)(m) and extracts
    diff_coeffs in Python.  Used only when section_poly_dict is absent.
    """
    tasks_by_prime = {}
    rhs_by_prime   = {}
...
def mumford_precompute_residues_oscar(
    ...
    When section_poly_dict is present: uses the new fast path — Julia does
    ladder + accumulation + diff poly + roots, no Sage point arithmetic in Python.

    When section_poly_dict is absent: falls back to the original task-building
    loop (Sage GF(p)(m) accumulation) for backward compatibility.
    """
    ...
    if use_new_path:
        ...
        vecs_by_prime, rhs_by_prime = _build_vecs_and_rhs(
            prime_list, vecs_lll, rhs_modp_list, vecs_list, const_val_int, debug,
        )
        tasks_by_prime = None
        if debug:
            print(f"[oscar_bridge] vec build (new path): {time.time()-t0:.3f}s", flush=True)
    else:
        # Old path: Sage accumulation.
        ...
        vecs_by_prime = None
        if debug:
            print(f"[oscar_bridge] task build (old path): {time.time()-t0:.2f}s", flush=True)
    ...
    julia_raw_str = _call_julia_server(
        ...
        tasks_by_prime,
        rhs_by_prime,
        section_poly_dict=section_poly_dict if use_new_path else None,
        vecs_by_prime=vecs_by_prime,
        debug=debug,
    )

    julia_raw = {}
    for p_str, pval in json.loads(julia_raw_str).items():
        julia_raw[int(p_str)] = pval
    ...
    if debug:
        ...
        print(
            f"[oscar_bridge] Julia: {time.time()-t1:.2f}s  ({total} m-roots)",
            flush=True,
        )
    ...
    for p in prime_list:
        ...
        for v_str, xmap in p_raw.items():
            ...
            for rhs_key_str, m_roots in xmap.items():
                rhs_key_obj = ast.literal_eval(rhs_key_str)
                if isinstance(rhs_key_obj, (list, tuple)):
                    rhs_idx = int(rhs_key_obj[0])
                else:
                    rhs_idx = int(rhs_key_obj)
