...
def estimate_galois_signature_modp(poly, primes_to_test=None, debug=DEBUG):
    ...
    estimate_galois_signature_modp.cache[key] = ret
    return ret
estimate_galois_signature_modp.cache = {}
...
def safe_compute_splitting_field_info_subprocess(poly, timeout=30, debug=DEBUG):
    ...
    """
    # Create cache key from polynomial string and timeout
    try:
        poly_str = str(poly)
        cache_key = (poly_str, timeout)
    except Exception as e:
        if debug:
...
def build_split_poly_from_cd(cd, debug=DEBUG):
    ...
    Cached based on cd.a4 and cd.a6 values.
    """
    try:
...
def auto_configure_search(cd, known_pts, prime_pool=None,
    ...
    Uses caching for expensive splitting field/Galois computations.
    """
    ...
    # Ensure small primes 2, 3, 5 are present if they were in the source
    for p in (2, 3, 5):
    ...
    if run_heavy_analysis and not USE_CONSENSUS_FILTER:
        try:
            ...
            galois_diag = _cached_galois_stats(str(poly))
            galois_degree = galois_diag.get('splitting_field_degree_est')
            if debug and galois_degree:
                print(f"[auto_cfg] Est. Splitting Field Deg: {galois_degree}")
        except Exception as e:
            if debug:
    ...
    if run_heavy_analysis:
        adapt_result = adaptive_prime_pool_empirical_survivor_count(
            ...
            max_iterations=150,
            max_extra=150,
            verbose=False # Keep it quiet unless debugging deep internals
        )
    ...
    # Ensure all primes in final pool have a residue count (even if default)
    ...
    tmax_scale = float(sqrt(height_bound / float(max(base_height_bound, 1))))
