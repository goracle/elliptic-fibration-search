    ...
    if precomputed_residues and precomputed_residues is not True: # precomputed_residues=True means only compute
        ...
        if first_prime and precomputed_residues[first_prime]:
            ...
            if all(v == 0 for v in first_vector):
                ...
                # IMPORTANT: We pass ONLY the zero vector to the worker. 
                # The worker will look up residues using this key. 
                # Since the precomputed map only has residues for this key, 
                # the worker will only execute CRT/Recon for this single vector,
                # saving massive overhead.
                
                dim = len(current_sections)
                zero_vec_tuple = tuple([0] * dim)
                
                # Ensure the zero vector tuple is in the original vecs list if it's supposed to be (it should always be
                # the Mordell-Weil basis represented by (0,0,...) is always included in the initial LLL search)
                if zero_vec_tuple in vecs:
                    optimized_vecs = [zero_vec_tuple] # Pass only the zero vector for the search loop
                    if debug:
                        print("OPTIMIZATION: Vector-Blind Consensus detected. Trimming search vectors (`vecs`) to just the zero vector.")
                else:
                    # Fallback to searching all vectors, but only the zero vector will yield residues
                    if debug:
                        print("WARNING: Vector-Blind Consensus detected, but zero vector not in original vecs. Proceeding without vector trimming.")
