...
def compute_height_pairing_simple(D1, D2, num_doublings=4):
    """
...
def build_mumford_basis_incremental(all_divisors, f_coeffs, debug=True):
    ...
    for i, (div, D) in enumerate(jac_elements):
        if not basis:
            ...
            h = compute_height_pairing_simple(D, D, num_doublings=4)
            if abs(h) < 1e-8:
                if debug:
                    print(f"[basis] Skipping divisor {i+1}: self-pairing too small ({h:.3g})")
                continue
            basis.append(div)
            basis_jac.append(D)
            if debug:
                print(f"[basis] Added divisor 1 (self-pairing {h:.3g})")
        else:
            ...
            for ii in range(n):
                for jj in range(ii, n):
                    h_ij = compute_height_pairing_simple(
                        candidate_basis[ii], 
                        candidate_basis[jj],
                        num_doublings=4
                    )
            ...
            if rank == n:
                ...
                basis_jac.append(D)
                if debug:
                    print(f"[basis] Added divisor {len(basis)} (rank {rank}/{n}, det {det:.3g})")
            else:
                if debug:
                    print(f"[basis] Skipping divisor {i+1}: dependent (rank {rank}/{n}, det {det:.3g})")
    
    rank = len(basis)
    
    # Build final height matrix
    if rank > 0:
        H = Matrix(RDF, rank, rank)
        for i in range(rank):
            for j in range(i, rank):
                h_ij = compute_height_pairing_simple(basis_jac[i], basis_jac[j], num_doublings=4)
                H[i, j] = h_ij
                H[j, i] = h_ij
        ...
        if debug:
            ...
            print(f"[basis] Determinant: {H.determinant():.3g}")
    else:
        H = None
