    ...
    with Timer("basis_construction"):
        for i, (div, D) in enumerate(jac_elements):
            if not basis:
                ...
                basis_indices.append(i)
                if debug:
                    print(f"[arakelov] Added divisor 1 (self-pairing {float(h_self):.6g})")
            else:
                candidate_indices = basis_indices + [i]
                m = len(candidate_indices)
                ...
                if rank == m and det_float > 0:
                    ...
                    basis_indices.append(i)
                    if debug:
                        print(f"[arakelov] Added divisor {len(basis)} (rank {rank}, det {det_float:.6g})")
                else:
                    if debug:
                        reason = "rank dropped" if rank < m else f"det non-positive"
                        print(f"[arakelov] Skipping divisor {i}: {reason}. det: {det_float:.6g}")
