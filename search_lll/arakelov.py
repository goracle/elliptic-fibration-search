...
def get_period_matrix(f_coeffs, prec=100):
    ...
    with Timer("period_matrix_total"):
        ...
        # Sort roots to define consistent cuts
        f_roots = sorted(f_roots, key=lambda z: (z.real(), z.imag()))
        
        with Timer("period_matrix_integration"):
            safe_t = log(prec) + RR_prec(2.0)
            max_t = RR_prec(safe_t)
            
            h = RR_prec(2.0 ** (-6))
            pi_half = RR_prec(pi) / RR_prec(2)
            ...
            def integrate_differential(root_start, root_end, use_x_weight):
                total = CC_prec(0)
                center = (root_start + root_end) / CC_prec(2)
                half_width = (root_end - root_start) / CC_prec(2)
                ...
                for (u, w) in nodes:
                    ...
                    if use_x_weight:
                        term_val = x_val / (CC_prec(2) * y_val)
                    else:
                        term_val = CC_prec(1) / (CC_prec(2) * y_val)
            ...
            # Standard symplectic basis for hyperelliptic genus 2:
            # a-cycles are integrals across the odd-indexed root gaps (the cuts)
            A[0,0] = CC_prec(2) * integrate_differential(f_roots[0], f_roots[1], False)
            A[1,0] = CC_prec(2) * integrate_differential(f_roots[0], f_roots[1], True)
            
            A[0,1] = CC_prec(2) * integrate_differential(f_roots[2], f_roots[3], False)
            A[1,1] = CC_prec(2) * integrate_differential(f_roots[2], f_roots[3], True)
            
            # b-cycles are paths connecting these cuts
            p1_diff0 = integrate_differential(f_roots[1], f_roots[2], False)
            p1_diff1 = integrate_differential(f_roots[1], f_roots[2], True)
            
            p2_diff0 = integrate_differential(f_roots[3], f_roots[4], False)
            p2_diff1 = integrate_differential(f_roots[3], f_roots[4], True)
            
            # Form B-periods such that B intersects a specific A cycle exactly once
            B[0,0] = p1_diff0
            B[1,0] = p1_diff1
            
            B[0,1] = p1_diff0 + p2_diff0
            B[1,1] = p1_diff1 + p2_diff1
...
def tanh_sinh_nodes(N):
    ...
    for k in range(-N, N+1):
        t = k * h
        x = tanh(pi/2 * sinh(t))       # node mapped to [-1,1]
        w = (pi/2) * cosh(t) / cosh(pi/2 * sinh(t))**2 * h  # weight
        nodes.append((x, w))
    return nodes

def test_period_matrix_pos_def_auto(f_coeffs, prec=300):
    print(f"\n--- Robust Period Matrix Test (prec={prec}) ---")
    nodes = tanh_sinh_nodes(2000)
    tau = get_period_matrix_auto_B(f_coeffs, prec=prec, nodes=nodes)
    RRp = RealField(prec)
    ...
    print("  Im(tau) det:", det)
    print("  eigenvalues:", evals)
    print("  positive definite:", is_pd)
    if not is_pd:
        raise AssertionError("Orientation or path selection is still wrong.")
    return True
...
"""
Drop-in replacement for get_period_matrix_auto_B in arakelov.py

This version correctly handles:
- Curves with 5 or 6 roots (genus 2)
- Complex conjugate pairs in branch points
...
# arakelov_fixed.py
# Complete fixed version of get_period_matrix_auto_B


# arakelov_fixed.py - Handles complex conjugate branch points correctly


# arakelov_fixed.py - Intelligent cut selection for genus 2


# arakelov_fixed.py - Intelligent cut selection for genus 2


# arakelov_complete.py - Full working version with multiple cut strategies


def get_period_matrix_auto_B(f_coeffs, prec=200, verbose=True):
    """
    Compute period matrix with automatic fallback to different cut selections.
    Tries multiple strategies until finding one that produces positive definite Im(τ).
    """
    ...
    if len(f_roots) < 5:
        raise ValueError(f"Need 5+ roots for genus 2, got {len(f_roots)}")
    
    # Sort by real part
    f_roots = sorted(f_roots, key=lambda z: (z.real(), z.imag()))
    
    # Identify real roots
    real_threshold = RR(10)**(-8)
    real_roots = [z for z in f_roots if abs(z.imag()) < real_threshold]
    
    if verbose:
        print(f"Found {len(real_roots)} real roots (of {len(f_roots)} total)")
    ...
    def tanh_sinh_nodes(N):
        ...
        for k in range(-N, N+1):
            t = k * h
            x_node = tanh(pi/2 * sinh(t))
            w = (pi/2) * cosh(t) / cosh(pi/2 * sinh(t))**2 * h
            nodes.append((RR(x_node), RR(w)))
        return nodes
    
    nodes = tanh_sinh_nodes(max(300, prec // 4))
    
    # Build list of cut strategies
    strategies = []
    
    if len(real_roots) >= 5:
        # Multiple strategies for 5+ real roots
        strategies.append(('real_0123', [real_roots[i] for i in [0,1,2,3]]))
        strategies.append(('real_0134', [real_roots[i] for i in [0,1,3,4]]))
        strategies.append(('real_0234', [real_roots[i] for i in [0,2,3,4]]))
        strategies.append(('real_1234', [real_roots[i] for i in [1,2,3,4]]))
    elif len(real_roots) >= 4:
        strategies.append(('real_first4', real_roots[:4]))
    elif len(real_roots) >= 2:
        # Mixed: 2 real + 2 closest to real axis
        remaining = [z for z in f_roots if z not in real_roots[:2]]
        remaining.sort(key=lambda z: abs(z.imag()))
        strategies.append(('mixed', real_roots[:2] + remaining[:2]))
    else:
        # All complex: use 4 closest to real axis
        candidates = sorted(f_roots, key=lambda z: abs(z.imag()))
        strategies.append(('complex', candidates[:4]))
    
    # Try each strategy
    last_error = None
    
    for strategy_name, e_list in strategies:
        e = sorted(e_list, key=lambda z: z.real())
        
        if verbose and len(strategies) > 1:
            print(f"\nStrategy '{strategy_name}': cuts at", [float(z.real()) for z in e])
        
        try:
            tau = _compute_period_matrix_for_cuts(e, f_coeffs, nodes, CC, RR, verbose and len(strategies) <= 1)
            if tau is not None:
                if verbose and len(strategies) > 1:
            ...
            last_error = ex
            if verbose and len(strategies) > 1:
                print(f"  Failed: {str(ex)[:80]}")
    
    # All strategies failed
    raise ArithmeticError(
        f"Failed all {len(strategies)} cut strategies. Last error: {last_error}"
    )


def _compute_period_matrix_for_cuts(e, f_coeffs, nodes, CC, RR, verbose):
    """
    Compute period matrix for specific choice of cuts.
    Returns tau if successful, None if Im(tau) not positive definite.
    """
    
    def f_at(x_val):
        return sum(CC(c) * x_val**(len(f_coeffs)-1-i) 
                   for i, c in enumerate(f_coeffs))
    
    def integrate_path(path_points, use_x_weight=False):
        """Integrate with continuous sqrt branch."""
        if not path_points:
            return CC(0)
        
        # Initialize on upper sheet
        x0, _, _ = path_points[0]
        f0 = f_at(x0)
        y0 = f0.sqrt()
        if y0.imag() < 0:
            y0 = -y0
        ...
        for x_val, dx_dt_val, t_weight in path_points:
            f_val = f_at(x_val)
            
            if abs(f_val) < CC(10)**(-len(str(CC.precision()))):
                continue
            
            # Continuous sqrt branch
            y_plus = f_val.sqrt()
            y_minus = -y_plus
            
            y_val = y_plus if abs(y_plus - y_prev) <= abs(y_minus - y_prev) else y_minus
            y_prev = y_val
            
            integrand = (x_val / (CC(2) * y_val)) if use_x_weight else (CC(1) / (CC(2) * y_val))
            total += integrand * dx_dt_val * CC(t_weight)
        
        return total
    
    # Build A-periods (around cuts)
    A = Matrix(CC, 2, 2)
    
    for cut_idx, (e_start, e_end) in enumerate([(e[0], e[1]), (e[2], e[3])]):
        offset_mag = max(RR(1e-8), abs(e_end - e_start) * RR(1e-6))
        offset = CC(0, offset_mag)
        
        def make_path(es=e_start, ee=e_end, off=offset):
            path = []
            for t_node, w in nodes:
                s = (CC(t_node) + CC(1)) / CC(2)
                x_val = es + s * (ee - es) + off
                dx_ds = ee - es
                dx_dt = dx_ds / CC(2)
                path.append((x_val, dx_dt, w))
            return path
        
        path = make_path()
        A[0, cut_idx] = CC(2) * integrate_path(path, use_x_weight=False)
        A[1, cut_idx] = CC(2) * integrate_path(path, use_x_weight=True)
    
    # Build B-periods (connecting paths)
    B = Matrix(CC, 2, 2)
    
    # B1: e[1] -> e[2]
    offset = CC(0, max(RR(1e-8), abs(e[2] - e[1]) * RR(1e-6)))
    
    def make_b1():
        path = []
        for t_node, w in nodes:
            s = (CC(t_node) + CC(1)) / CC(2)
            x_val = e[1] + s * (e[2] - e[1]) + offset
            dx_ds = e[2] - e[1]
            dx_dt = dx_ds / CC(2)
            path.append((x_val, dx_dt, w))
        return path
    ...
    b1 = make_b1()
    B[0, 0] = integrate_path(b1, use_x_weight=False)
    B[1, 0] = integrate_path(b1, use_x_weight=True)
    
    # B2: e[3] -> large point (or e[4] if exists)
    # Get all roots again from f_coeffs
    R = PolynomialRing(QQ, 'x')
    x = R.gen()
    f_poly = sum(QQ(c) * x**(len(f_coeffs)-1-i) for i, c in enumerate(f_coeffs))
    all_roots = sorted(f_poly.roots(CC, multiplicities=False), key=lambda z: z.real())
    
    # Find e[4] - the next root after e[3]
    e3_idx = None
    for i, root in enumerate(all_roots):
        if abs(root - e[3]) < RR(1e-10):
            e3_idx = i
            break
    
    if e3_idx is not None and e3_idx + 1 < len(all_roots):
        e_far = all_roots[e3_idx + 1]
    else:
        # Use large point in upper half plane
        e_far = e[3] + CC(50, 50)
    
    offset = CC(0, max(RR(1e-8), abs(e_far - e[3]) * RR(1e-6)))
    
    def make_b2():
        path = []
        for t_node, w in nodes:
            s = (CC(t_node) + CC(1)) / CC(2)
            x_val = e[3] + s * (e_far - e[3]) + offset
            dx_ds = e_far - e[3]
            dx_dt = dx_ds / CC(2)
            path.append((x_val, dx_dt, w))
        return path
    ...
    b2 = make_b2()
    B[0, 1] = integrate_path(b2, use_x_weight=False)
    B[1, 1] = integrate_path(b2, use_x_weight=True)
    ...
    if verbose:
        print(f"Im(τ) eigenvalues: {[float(e) for e in evals]}")
    
    # Try orientation fixes
    num_neg = sum(1 for e in evals if e < 0)
    ...
    if num_neg == 2:
        ...
        # Try all 16 combinations of transformations
        best_tau = None
        best_min_eval = -float('inf')
            ...
            for flip_0 in [1, -1]:
                for flip_1 in [1, -1]:
                    for global_sign in [1, -1]:
                        ...
                        tau_try = global_sign * (A_inv * B_new)
                        tau_try = (tau_try + tau_try.transpose()) / CC(2)
                        
                        Im_try = Matrix(RR, 2, 2, [[RR(tau_try[i,j].imag()) for j in range(2)] for i in range(2)])
                        
                        try:
                            evals_try = Im_try.eigenvalues()
                            min_eval = min(evals_try)
                            
                            if min_eval > best_min_eval:
                                best_min_eval = min_eval
                                best_tau = tau_try
                            
                            if all(e > RR(1e-10) for e in evals_try):
                                if verbose:
                                    print(f"✓ Fixed: swap={swap_cols}, flips=({flip_0},{flip_1}), sign={global_sign}")
                                return tau_try
                        except:
                            pass
        
        # Use best attempt even if not perfect
        if best_tau is not None and best_min_eval > -RR(1):
            tau = best_tau
            Im_tau = Matrix(RR, 2, 2, [[RR(tau[i,j].imag()) for j in range(2)] for i in range(2)])
            evals = Im_tau.eigenvalues()
    
    # Final check
    if all(e > RR(1e-10) for e in evals):
        if verbose:
            print(f"✓ Success: {[float(e) for e in evals]}")
        return tau
