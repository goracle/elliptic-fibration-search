...
def compute_rank_via_torsion_projection(elements_p, J_p, debug=False):
    ...
    """
    if not elements_p:
        raise ValueError("elements_p cannot be empty")
    ...
    try:
        ...
        if debug:
            ...
            print(f"[torsion rank] Falling back to simpler rank check")
        ...
        try:
            ...
            for D in nonzero:
                ...
                try:
                    ...
                    if debug:
                        print(f"[torsion rank] l={l}: scalar mult failed for divisor: {e}")
                    continue
                ...
                try:
                    ...
                    if debug:
                        print(f"[torsion rank] l={l}: torsion check failed: {e}")
                    continue
...
def fallback_rank_check(elements_p, J_p, debug=False):
    """
    Simpler rank check when Kedlaya fails or torsion primes unavailable.
    Uses direct linear algebra over a small torsion subgroup.
    """
    if debug:
...
def jacobian_group_order(J_p, prec=2):
    """
    Compute #J(F_p) via Kedlaya (Monsky-Washnitzer).
    """
    try:
        C = J_p.curve()
    except Exception as e:
    ...
    try:
        ...
        raise RuntimeError(f"jacobian_group_order: failed to determine characteristic: {e}")

    if p < 5:
        raise ValueError("Kedlaya requires p >= 5")
        ...
        try:
            ...
            if lc is not None:
                msg += (
                    ...
                    f"ring (e.g. divisible by p)."
                )
            raise RuntimeError(msg + f" Original error: {e}")
