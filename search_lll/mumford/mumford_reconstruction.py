...
def _reconstruct_mumford_finite_field(residues, f_coeffs, shift, rationality_test, debug):
    """
    Streaming reconstruction with HARD memory caps and generator-based processing.
    """
    from collections import defaultdict
    import math
    from sage.all import matrix, GF, ZZ
    ...
    # GENERATOR-based work item creation
    def work_generator():
        """Generator that yields work batches on-demand, never materializing full list."""
        for v_tuple in vector_list:
            if vector_stats[v_tuple]['collected'] >= MAX_RELATIONS_PER_VECTOR:
                continue
            ...
            # Process in chunks without storing full list
            for i in range(0, len(sols_list), BATCH_SIZE):
                if vector_stats[v_tuple]['collected'] >= MAX_RELATIONS_PER_VECTOR:
                    break
                if len(seen_supports) >= MAX_TOTAL_RELATIONS:
                    return  # Global memory cap hit
                
                batch = sols_list[i:i+BATCH_SIZE]
                if batch:
                    yield (batch, f_coeffs, p, shift, v_tuple)
    
    num_workers = min(8, max(4, multiprocessing.cpu_count()))
    
    if debug:
        print(f"  Using spawn context with {num_workers} workers (memory-bounded streaming)")
    ...
    try:
        ...
        # Use imap_unordered with generator - NEVER materializes full batch list
        for batch_divs, batch_stats in pool.imap_unordered(_ff_worker_verify_batch, work_generator(), chunksize=1):
            ...
            for div_entry in batch_divs:
                if len(seen_supports) >= MAX_TOTAL_RELATIONS:
                    if debug:
                        print(f"\n  [MEMORY CAP] Hit MAX_TOTAL_RELATIONS={MAX_TOTAL_RELATIONS}")
                    should_stop = True
                    break
